...
class RRDBNet(nn.Module):
    def __init__(self, in_nc=3, out_nc=3, nc=64, nb=23, gc=32, sf=4):
        super(RRDBNet, self).__init__()
        RRDB_block_f = functools.partial(RRDB, nc=nc, gc=gc)
        self.sf = sf
        ...
        #### upsampling
        self.upconv1 = nn.Conv2d(nc, nc, 3, 1, 1, bias=True)
        if self.sf==4:
            self.upconv2 = nn.Conv2d(nc, nc, 3, 1, 1, bias=True)
