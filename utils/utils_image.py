...
def split_imageset(original_dataroot, target_dataroot, n_channels=3, p_size=512, p_overlap=96, p_max=800):
    """
...
'''
# --------------------------------------------
# convolution, gray image or multichannels
# --------------------------------------------
'''
def convolution(img, psf):
    # single channel convolution
    assert img.dtype == 'uint8' and psf.dtype == 'float32'
    assert len(img.shape) == 2 and len(psf.shape) == 2 # gray scale image
    img = ndimage.convolve(uint2single(img), psf, mode='reflect')
    
    return img

def convolution_mc(img, psf):
    # multichannel convolution
    assert img.dtype == 'uint8' and psf.dtype == 'float32'
    assert len(img.shape) == 3 and len(psf.shape) == 3 # multichannel scale image
    img = uint2single(img)
    img_r = ndimage.convolve(img[:, :, 0], psf[:, :, 0], mode='reflect')
    img_g = ndimage.convolve(img[:, :, 1], psf[:, :, 1], mode='reflect')
    img_b = ndimage.convolve(img[:, :, 2], psf[:, :, 2], mode='reflect')
    img = np.stack((img_r, img_g, img_b), 2)
